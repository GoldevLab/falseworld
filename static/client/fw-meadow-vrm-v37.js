/**
 * False World — VRM overlay v37 (Rust/Fortnite loco + Mixamo for FPV & 3rd person).
...
async function create(canvas, opts) {
  ...
  function setFpvHeadVisible(show) {
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      // Also hide child meshes parented under head hierarchy
      bone.visible = show;
    }
    fpvHeadHidden = !show;
  }
  ...
  function applyVisibility() {
    ...
    // Visible in FPV + follow + orbit (C). FPV hides head to avoid clip.
    const hide = !playable || !modelReady;
    vrm.scene.visible = !hide;
    if (!hide) {
      const fpv = pose?.camMode === "fpv";
      if (fpv && !fpvHeadHidden) setFpvHeadVisible(false);
      else if (!fpv && fpvHeadHidden) setFpvHeadVisible(true);
    }
  }
