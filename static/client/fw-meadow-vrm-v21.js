/**
 * False World — VRM overlay v21 (depth atlas for grass occlusion).
...
async function create(canvas, opts) {
  ...
  function syncAnimWeights(moving, dt) {
    ...
    // Smooth blend; both clips keep their time running (never re-play → no restart)
    walkBlend += (target - walkBlend) * Math.min(1, dt * 8);
    ...
    idle.paused = false;
    walk.paused = false;
    // IMPORTANT: do not call play() here. Three's isRunning() is false at weight 0,
    // and play() re-schedules the clip from t=0 every time you start walking.
  ...
  try {
    ...
    walk.play(); // keep running under weight 0 so resume is mid-stride
    walk.time = 0; // only initial; never reset again while alive
  } catch (we) {
