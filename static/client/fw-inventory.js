  ...
  window.FalseWorldInv = {
    create(host, opts) {
      ...
      root.innerHTML =
        ...
        '      <div class="fw-player-name" id="fw-inv-player-name">FALSE WORLD</div>' +
        '      <div class="fw-player-avatar">' +
        '        <canvas id="fw-inv-avatar" class="fw-inv-avatar" width="288" height="448" aria-label="Avatar"></canvas>' +
        '        <div class="fw-inv-avatar-fallback" id="fw-inv-avatar-fallback">Cargando avatar…</div>' +
        '      </div>' +
        '      <div class="fw-player-stats">' +
        '        <span id="fw-inv-hp" title="Salud">—</span>' +
        '        <span id="fw-inv-hunger" title="Hambre">—</span>' +
        '        <span id="fw-inv-comfort" title="Confort">—</span>' +
        '      </div>' +
        '    </section>' +
      ...
      const avatarCanvas = root.querySelector("#fw-inv-avatar");
      const avatarFallback = root.querySelector("#fw-inv-avatar-fallback");
      const playerNameEl = root.querySelector("#fw-inv-player-name");
      let portraitRaf = 0;
      let portraitAcc = 0;

      function syncPlayerName() {
        if (!playerNameEl) return;
        let name = "FALSE WORLD";
        try {
          if (window.__fw && window.__fw.playerName) name = String(window.__fw.playerName);
        } catch (_) {}
      ...
      function tickPortrait(now) {
        portraitRaf = 0;
        if (!bagOpen) return;
        portraitRaf = requestAnimationFrame(tickPortrait);
        // ~12 fps is enough for a static inventory pose
        if (now - portraitAcc < 80) return;
        portraitAcc = now;
        let ok = false;
        try {
          const vrm = window.__fw && window.__fw.vrm;
          if (vrm && typeof vrm.drawPortrait === "function" && avatarCanvas) {
            ok = !!vrm.drawPortrait(avatarCanvas);
          }
        } catch (_) {}
        if (avatarCanvas) avatarCanvas.classList.toggle("is-on", ok);
        if (avatarFallback) avatarFallback.hidden = !!ok;
      }
      ...
      function startPortrait() {
        ...
        if (avatarFallback) {
          ...
          avatarFallback.textContent = "Cargando avatar…";
        }
        if (avatarCanvas) avatarCanvas.classList.remove("is-on");
        portraitRaf = requestAnimationFrame(tickPortrait);
      }
      ...
      function renderSlot(el, stack, idx, isHot, selected) {
        el.className = "fw-slot"
          + (isHot ? " is-hot" : "")
          + (selected ? " is-active" : "")
          + (stack ? " has-item" : " is-empty");
        el.dataset.idx = String(idx);
        el.dataset.kind = isHot ? "hot" : "bag";
        if (!stack) {
          el.removeAttribute("title");
          el.innerHTML = isHot
            ? '<span class="fw-slot-num">' + (idx + 1) + '</span><span class="fw-slot-empty" aria-hidden="true"></span>'
            : '<span class="fw-slot-empty" aria-hidden="true"></span>';
          return;
        }
      ...
      function syncInvVitals() {
        let v = null;
        try {
          if (window.__fw && typeof window.__fw.getVitals === "function") v = window.__fw.getVitals();
          else if (window.__fw && window.__fw.api && typeof window.__fw.api.getVitals === "function") {
            v = window.__fw.api.getVitals();
          }
        } catch (_) {}
        const hpEl = root.querySelector("#fw-inv-hp");
        const huEl = root.querySelector("#fw-inv-hunger");
        const coEl = root.querySelector("#fw-inv-comfort");
        if (!hpEl || !huEl || !coEl) return;
        if (!v) {
          hpEl.textContent = "—";
          huEl.textContent = "—";
          coEl.textContent = "—";
        ...
        hpEl.textContent = String(Math.ceil(v.hp != null ? v.hp : 0));
        huEl.textContent = String(Math.ceil(v.hunger != null ? v.hunger : 0));
        coEl.textContent = String(Math.round((v.comfort != null ? v.comfort : 0) * 100));
      }
      ...
      function setVitalsWatch(on) {
        ...
        if (on) {
          syncInvVitals();
          vitalsTimer = setInterval(syncInvVitals, 400);
        }
      }
      ...
      function toggleBag() {
        ...
        paint();
        setVitalsWatch(bagOpen);
        if (bagOpen) startPortrait();
        else stopPortrait();
        notifyBagUi();
        try {
          if (window.__fw && typeof window.__fw.playUi === "function") {
            window.__fw.playUi(bagOpen ? "bag_open" : "bag_close");
          }
        } catch (_) {}
      }
      ...
      function closeBag() {
        ...
        bagFocus = -1;
        paint();
        setVitalsWatch(false);
        stopPortrait();
        notifyBagUi();
        try {
          if (window.__fw && typeof window.__fw.playUi === "function") {
            window.__fw.playUi("bag_close");
          }
        } catch (_) {}
      }
      ...
      // Icons may load/bake after mount (module order / WebGL). Repaint until urls appear.
      (async () => {
        for (let i = 0; i < 50; i++) {
          try {
            const api = window.FalseWorldItemIcons;
            if (api && typeof api.ready === "function") await api.ready();
            paint();
            if (api && typeof api.has === "function" && api.has("hatchet_tool")) break;
          } catch (_) {}
