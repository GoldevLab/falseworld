/**
 * False World — VRM overlay v7 (VRMedia fetch + deferred parse).
 * Download blocks the gate; parse/anims run AFTER create() returns so START no se congela.
 */
...
async function create(canvas, opts) {
  ...
  // Kick heavy work without blocking create() return
  const heavy = finishHeavyWork().catch((e) => {
  ...
  return {
    ...
    setPlayable(on) {
      ...
      else heavy.then(() => applyVisibility());
    },
    destroy() {
      alive = false;
