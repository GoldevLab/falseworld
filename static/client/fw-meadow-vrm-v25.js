/**
 * False World — VRM overlay v25 (depth atlas for grass occlusion).
  ...
  for (const track of clip.tracks) {
    ...
    if (track instanceof THREE.QuaternionKeyframeTrack) {
      ...
      for (let i = 0; i < track.values.length; i += 3) {
        if (vrmBoneName === "hips") {
          // In-place locomotion: kill Mixamo root-motion XZ.
          // Otherwise LoopRepeat snaps hips back to clip start every cycle.
          value[i] = 0;
          value[i + 1] = vrmHipsHeight;
          value[i + 2] = 0;
        } else {
...
async function create(canvas, opts) {
  ...
  function syncAnimWeights(moving, dt) {
    ...
    // Smooth blend; both clips keep looping — walk time never reset
    walkBlend += (target - walkBlend) * Math.min(1, dt * 7);
    walkBlend = Math.min(1, Math.max(0, walkBlend));
    idle.enabled = true;
    walk.enabled = true;
    idle.paused = false;
    walk.paused = false;
    // Keep a tiny floor weight so the mixer never treats walk as fully inactive
    // (avoids some Three versions rewinding on 0→positive weight).
    const wWalk = walkBlend < 0.001 ? 0.0001 : walkBlend;
    const wIdle = 1 - Math.min(walkBlend, 0.9999);
  ...
  try {
    ...
    walk.play(); // keep running under near-zero weight so resume is mid-stride
  } catch (we) {
