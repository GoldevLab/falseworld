...
(function () {
  ...
  function create() {
    ...
    function handleMsg(raw) {
      ...
      if (msg.t === "world" || msg.t === "place" || msg.t === "remove" || msg.t === "hp") {
        emitWorld(msg);
      }
    }
    ...
    return {
      ...
      // Server resolves damage against its authoritative HP (not our local
      // number) and broadcasts the corrected `hp`/`remove` back to everyone —
      // this is what keeps a raid's wall HP identical for every peer.
      sendExplode(kind, hits) {
        if (!ws || ws.readyState !== 1 || !Array.isArray(hits) || !hits.length) return;
        try {
          ws.send(JSON.stringify({
            t: "explode",
            kind: String(kind || "satchel"),
            hits: hits.slice(0, 48).map((h) => ({ id: String(h.id), soft: !!h.soft })),
          }));
        } catch (_) {}
      },
      onWorld(fn) {
