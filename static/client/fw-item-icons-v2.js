...
function meshHatchet() {
  const g = new THREE.Group();
  const wood = mat(0x6e3f1a, { roughness: 0.88 });
  const steel = mat(0xc5ccd3, { roughness: 0.32, metalness: 0.9 });
  const handle = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.08, 1.25, 10), wood);
  handle.rotation.z = 0.55;
  handle.position.set(-0.05, -0.15, 0);
  const blade = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.42, 0.08), steel);
  blade.position.set(0.38, 0.42, 0);
  blade.rotation.z = 0.55;
  const edge = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.5, 0.04), steel);
  edge.position.set(0.62, 0.55, 0);
  edge.rotation.z = 0.55;
  const poll = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.28, 0.14), steel);
  poll.position.set(0.12, 0.28, 0);
  poll.rotation.z = 0.55;
  g.add(handle, blade, edge, poll);
