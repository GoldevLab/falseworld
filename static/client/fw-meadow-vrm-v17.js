/**
 * False World — VRM overlay v12 (context lost recovery).
...
async function create(canvas, opts) {
  ...
  function setGait(next) {
    ...
    if (next === "walk") {
      // Continue walk cycle — never reset() (that rewound to frame 0 every start)
      walk.enabled = true;
      walk.paused = false;
      ...
      idle.fadeIn(0.25);
      walk.fadeOut(0.2);
      // walk stays enabled so its time keeps advancing while faded (seamless resume)
    }
    gait = next;
  }
  ...
  function tick() {
    ...
    if (pose && vrm && modelReady) {
      ...
      if (vrm.scene.visible) {
        ...
        // Follow cam is behind yaw-forward; +π so we see the back (Sophia faces -Z at 0)
  ...
  try {
    ...
    walk.play(); // start clock immediately so resume never jumps to 0
  } catch (we) {
