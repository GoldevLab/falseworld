/** False World — Three.js baked inventory + build-piece icons (mini 3D renders). v4 tools. */
...
function meshPickaxe() {
  const g = new THREE.Group();
  const wood = mat(0x6e4320, { roughness: 0.88 });
  const woodLite = mat(0x9a6a38, { roughness: 0.78 });
  const steel = mat(0xc5ccd4, { roughness: 0.28, metalness: 0.92 });
  const steelDark = mat(0x6a727c, { roughness: 0.4, metalness: 0.88 });
  // Tapered ash handle
  const handle = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.085, 1.72, 12), wood);
  handle.rotation.z = 0.52;
  handle.position.set(-0.22, -0.18, 0);
  const grip = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.075, 0.42, 12), woodLite);
  grip.rotation.z = 0.52;
  grip.position.set(-0.55, -0.52, 0);
  const pommel = new THREE.Mesh(new THREE.SphereGeometry(0.09, 10, 8), wood);
  pommel.position.set(-0.78, -0.72, 0);
  // Cross peen / eye
  const eye = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 0.2, 10), steelDark);
  eye.rotation.x = Math.PI * 0.5;
  eye.position.set(0.38, 0.42, 0);
  eye.rotation.z = 0.52;
  // Curved dual pick head
  const headCore = new THREE.Mesh(new THREE.BoxGeometry(0.95, 0.16, 0.2), steel);
  headCore.position.set(0.42, 0.48, 0);
  headCore.rotation.z = 0.52;
  // Pointy pick tip (front)
  const tip = new THREE.Mesh(new THREE.ConeGeometry(0.11, 0.62, 8), steel);
  tip.position.set(0.92, 0.88, 0);
  tip.rotation.z = -0.95;
  // Chisel / adze butt
  const butt = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.14, 0.18), steelDark);
  butt.position.set(0.02, 0.18, 0);
  butt.rotation.z = 0.52;
  const spike = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.38, 7), steel);
  spike.position.set(-0.18, 0.02, 0);
  spike.rotation.z = 2.2;
  // Binding ring
  const ring = new THREE.Mesh(new THREE.TorusGeometry(0.09, 0.025, 6, 14), steelDark);
  ring.position.set(0.22, 0.28, 0);
  ring.rotation.y = Math.PI * 0.5;
  ring.rotation.z = 0.52;
  g.add(handle, grip, pommel, eye, headCore, tip, butt, spike, ring);
  g.rotation.y = 0.35;
...
function meshHatchet() {
  const g = new THREE.Group();
  const wood = mat(0x5c3818, { roughness: 0.9 });
  const woodLite = mat(0x8a5a28, { roughness: 0.82 });
  const steel = mat(0xd8dde3, { roughness: 0.22, metalness: 0.95 });
  const steelDark = mat(0x5a636e, { roughness: 0.35, metalness: 0.9 });
  // Handle along X — clear axe silhouette
  const handle = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.072, 1.65, 12), wood);
  handle.rotation.z = Math.PI * 0.5;
  handle.position.set(-0.12, 0.02, 0);
  const grip = new THREE.Mesh(new THREE.CylinderGeometry(0.065, 0.07, 0.38, 12), woodLite);
  grip.rotation.z = Math.PI * 0.5;
  grip.position.set(-0.72, 0.02, 0);
  const butt = new THREE.Mesh(new THREE.SphereGeometry(0.085, 10, 8), wood);
  butt.position.set(-0.98, 0.02, 0);
  // Poll / cheek
  const cheek = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.42, 0.2), steelDark);
  cheek.position.set(0.58, 0.06, 0);
  // Main bit — wide flat blade that reads as axe
  const blade = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.78, 0.07), steel);
  blade.position.set(0.88, 0.1, 0);
  // Bevel edge (thinner, taller)
  const edge = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.95, 0.035), steel);
  edge.position.set(1.14, 0.12, 0);
  // Toe / heel flare for classic hatchet profile
  const toe = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.22, 0.05), steel);
  toe.position.set(1.05, 0.52, 0);
  toe.rotation.z = -0.35;
  const heel = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.2, 0.05), steel);
  heel.position.set(1.02, -0.28, 0);
  heel.rotation.z = 0.4;
  // Eye wedge + ring
  const eye = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.22, 0.22), steelDark);
  eye.position.set(0.48, 0.04, 0);
  const ring = new THREE.Mesh(new THREE.TorusGeometry(0.085, 0.022, 6, 12), steel);
  ring.rotation.y = Math.PI * 0.5;
  ring.position.set(0.35, 0.02, 0);
  g.add(handle, grip, butt, cheek, blade, edge, toe, heel, eye, ring);
  g.rotation.z = -0.4;
  g.rotation.y = 0.65;
...
function meshHammer() {
  const g = new THREE.Group();
  const wood = mat(0x6a401c, { roughness: 0.88 });
  const woodLite = mat(0x956530, { roughness: 0.8 });
  const steel = mat(0xb8c0c8, { roughness: 0.3, metalness: 0.9 });
  const steelDark = mat(0x4e5660, { roughness: 0.38, metalness: 0.88 });
  // Carpenter claw-hammer silhouette
  const handle = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.08, 1.45, 12), wood);
  handle.rotation.z = 0.42;
  handle.position.set(-0.18, -0.15, 0);
  const grip = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.08, 0.4, 12), woodLite);
  grip.rotation.z = 0.42;
  grip.position.set(-0.48, -0.48, 0);
  const pommel = new THREE.Mesh(new THREE.SphereGeometry(0.085, 10, 8), wood);
  pommel.position.set(-0.68, -0.68, 0);
  // Head body
  const head = new THREE.Mesh(new THREE.BoxGeometry(0.62, 0.3, 0.3), steel);
  head.position.set(0.32, 0.42, 0);
  head.rotation.z = 0.42;
  // Striking face (slightly proud cylinder)
  const face = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.15, 0.12, 14), steelDark);
  face.rotation.z = Math.PI * 0.5 + 0.42;
  face.position.set(0.58, 0.58, 0);
  // Claw — split V
  const clawL = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.1, 0.08), steel);
  clawL.position.set(0.02, 0.58, 0.07);
  clawL.rotation.z = 1.05;
  const clawR = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.1, 0.08), steel);
  clawR.position.set(0.02, 0.58, -0.07);
  clawR.rotation.z = 1.05;
  const clawBridge = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.12, 0.28), steelDark);
  clawBridge.position.set(0.12, 0.5, 0);
  clawBridge.rotation.z = 0.42;
  // Cheek / neck
  const neck = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.22, 0.22), steelDark);
  neck.position.set(0.18, 0.28, 0);
  neck.rotation.z = 0.42;
  const ring = new THREE.Mesh(new THREE.TorusGeometry(0.09, 0.022, 6, 12), steel);
  ring.position.set(0.05, 0.18, 0);
  ring.rotation.y = Math.PI * 0.5;
  ring.rotation.z = 0.42;
  g.add(handle, grip, pommel, head, face, clawL, clawR, clawBridge, neck, ring);
  g.rotation.y = 0.4;
