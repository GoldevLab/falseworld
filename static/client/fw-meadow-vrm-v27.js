/**
 * False World — VRM overlay v27 (feet on ground + depth composite bias).
...
async function create(canvas, opts) {
  ...
  function plantFeetOnGround(groundY) {
    ...
    // Slight lift so soles sit on top of the heightmap (not buried / depth-tied)
    const lift = 0.045;
    vrm.scene.position.y += groundY - box.min.y + lift;
  }
