/**
 * False World — VRM overlay v9 (VRMedia pipeline).
...
async function create(canvas, opts) {
  ...
  // VRMedia AvatarViewer: parse immediately after fetch (not deferred).
  // Yield once so the gate can paint "parseando…" before the sync GLTF work.
  ...
  const gltf = await new Promise((resolve, reject) => {
    try {
      // Same as VRMedia: path '' — textures are bufferViews inside the GLB.
