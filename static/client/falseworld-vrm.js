/**
 * False World — VRM overlay (Three.js WebGL + @pixiv/three-vrm).
 * Pattern from VRMedia (GoldevLab): Mixamo idle/walk retarget onto VRM.
 * Meadow stays WebGPU; this canvas is transparent and mirrors FalseWorldGpu.getPose().
 */
(function () {
  ...
  const DEFAULTS = {
    ...
    idleUrl: "/animaciones/Standing%20Idle.fbx",
    walkUrl: "/animaciones/Walking.fbx",
  };

  function processMixamoClip(THREE, asset, vrm) {
  ...
  async function create(canvas, opts) {
    ...
    const { VRMLoaderPlugin, VRMUtils } = await import("https://esm.sh/@pixiv/three-vrm@3.4.3");
    ...
    const sun = new THREE.DirectionalLight(0xc8d8ff, 0.85);
    ...
    const gltf = await new Promise((resolve, reject) => {
      loader.load(opts.vrmUrl, resolve, undefined, reject);
    });
    ...
    async function loadFbxClip(url) {
      ...
      return processMixamoClip(THREE, asset, vrm);
    }

    const mixer = new THREE.AnimationMixer(vrm.scene);
    const [idleClip, walkClip] = await Promise.all([
      loadFbxClip(opts.idleUrl),
      loadFbxClip(opts.walkUrl),
    ]);
    ...
    function tick() {
      ...
      if (pose) {
        ...
        if (!hide) {
          ...
          // VRM faces −Z; meadow yaw 0 looks +Z → flip π
