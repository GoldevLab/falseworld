/**
 * False World — VRM overlay v10 (parse timeout + rAF after parse).
...
async function create(canvas, opts) {
  ...
  // VRMedia: parse after fetch. Timeout — WebGPU+WebGL can stall texture decode.
  onProgress("VRM · parseando…");
  await yieldFrame();
  await new Promise((r) => requestAnimationFrame(r));

  const PARSE_MS = 45000;
  const gltf = await new Promise((resolve, reject) => {
    let settled = false;
    const timer = setTimeout(() => {
      if (settled) return;
      settled = true;
      reject(new Error("VRM parse timeout (" + PARSE_MS + "ms)"));
    }, PARSE_MS);
    try {
      loader.parse(
        arrayBuffer,
        ...
        (error) => {
          ...
          settled = true;
          clearTimeout(timer);
          reject(error || new Error("VRM parse failed"));
        },
      );
    } catch (e) {
      if (settled) return;
