/**
 * False World — VRM overlay v39 (Rust/Fortnite loco + Mixamo for FPV & 3rd person).
 * Crouch/walk/run/jump/swim; FPV body+hands visible, head bone collapsed to avoid camera clip; gather LoopRepeat.
...
async function create(canvas, opts) {
  ...
  function setFpvHeadCollapsed(collapsed) {
    ...
    // Skinned VRM: scale head bones → 0 so face vanishes but body/arms stay (FPS trick)
    const s = collapsed ? 0.001 : 1;
    const names = ["head", "leftEye", "rightEye", "jaw"];
    ...
    // Soften neck so camera doesn't clip hair/collar as much
    const neck = vrm.humanoid.getNormalizedBoneNode("neck");
    if (neck) neck.scale.setScalar(collapsed ? 0.15 : 1);
    try {
      const rawNeck = vrm.humanoid.getRawBoneNode && vrm.humanoid.getRawBoneNode("neck");
      if (rawNeck) rawNeck.scale.setScalar(collapsed ? 0.15 : 1);
    } catch (_) {}
  ...
  function tick() {
    ...
    // Head/eyes track mouse / build ghost — skip in FPV (head is collapsed)
