...
(function () {
  ...
  window.FalseWorldProgression = {
    ...
    create(host, opts) {
      ...
      function uiSfx(kind) {
        try {
          if (window.__fw && typeof window.__fw.playUi === "function") window.__fw.playUi(kind);
        } catch (_) {}
      }
      ...
      function researchItem(itemId) {
        ...
        try {
          ...
          window.__fw.researchBusyUntil = performance.now() + 1200;
        } catch (_) {}
        onHud({ status: "Investigando · " + itemId + "…" });
        paint();
        setTimeout(() => {
          unlocked.add(itemId);
          for (const n of allTechNodes()) {
          ...
          researchBusy = false;
          try {
            if (window.__fw) window.__fw.researchBusyUntil = 0;
          } catch (_) {}
          paint();
        }, 1100);
        return true;
      ...
      function paintResearch() {
        ...
        for (let i = 0; i < ids.length; i++) {
          ...
          if (researchBusy) cls += " is-busy";
          html +=
            '<button type="button" class="fw-prog-card ' + cls + '" data-res="' + id + '"' +
            (owned || researchBusy ? " disabled" : "") + ">" +
            (iconUrl(id)
        ...
        html += '<button type="button" class="fw-prog-start' + (researchBusy ? " is-busy" : "") +
          '" data-act="start"' +
          (!researchPick || researchBusy ? " disabled" : "") + ">" +
          (researchBusy ? "Investigando…" : "Comenzar Investigación") + "</button>";
        html += "</div>";
        body.innerHTML = html;
        if (!researchBusy) {
        ...
        if (start && !researchBusy) {
          start.addEventListener("click", () => {
            if (researchPick) researchItem(researchPick);
          });
        }
        bindRmbTips(body);
        foot.textContent = researchBusy
          ? "Investigando plano…"
          : "E abre esta mesa · Tab cierra · el objeto se sacrifica al aprender";
      }
